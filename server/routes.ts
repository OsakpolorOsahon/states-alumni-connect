...
import { insertMemberSchema, insertUserSchema, insertBadgeSchema, insertHallOfFameSchema, insertNewsSchema, insertForumThreadSchema, insertForumReplySchema, insertJobPostSchema, insertJobApplicationSchema, insertMentorshipRequestSchema, insertNotificationSchema, insertEventSchema } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      ...
      if (!username || !password || !memberData) {
        return res.status(400).json({ error: "Username, password, and member data are required" });
      }
      ...
      res.status(201).json({ user: { id: user.id, username: user.username }, member });
    } catch (error) {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      res.json({ user: { id: user.id, username: user.username }, member });
    } catch (error) {
  ...
  app.get("/api/auth/me", async (req, res) => {
    try {
      ...
      const member = await storage.getMemberByUserId(user.id.toString());
      req.session.member = member; // Update session with latest member data
  ...
  // Middleware to require secretary role
  const requireSecretary = (req: any, res: any, next: any) => {
    if (!req.session.member || req.session.member.role !== 'secretary') {
      return res.status(403).json({ error: "Secretary role required" });
    }
    next();
  };
  // Stats endpoint
